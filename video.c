...
static int video_message_block(void)
{
	...
	if( !info.msg_lock ) {
		log_qcy(DEBUG_VERBOSE, "===video message block, return 0 when first message is msg_lock=0");
		return 0;
	}
	index = 0;
	msg_init(&msg);
	...
	if( ret ) {
		log_qcy(DEBUG_VERBOSE, "===video message block, return 0 when first message is empty");
		return 0;
	}
	if( msg_is_system(msg.message) || msg_is_response(msg.message) ) {
		log_qcy(DEBUG_VERBOSE, "===video message block, return 0 when first message is system or response message %x", msg.message);
		return 0;
	}
	id = msg.message;
	do {
		...
		ret = msg_buffer_probe_item(&message, index, &msg);
		if(ret) {
			log_qcy(DEBUG_VERBOSE, "===video message block, return 1 when message index = %d is not found!", index);
			return 1;
		}
		if( msg_is_system(msg.message) ||
			...
			id1 = msg.message;
			log_qcy(DEBUG_INFO, "video: swapped message happend, message %x was swapped with message %x", id, id1);
			return 0;
		}
	}
	while(!ret);
